package com.pandora.backend.error;

import com.pandora.backend.error.exception.InvalidArgumentException;
import com.pandora.backend.error.exception.ResourceFetchException;
...
public class RestExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleError404(Exception e) {
        return buildResponseEntity(new ApiError(HttpStatus.NOT_FOUND, "Requested resource does not exist",
    ...
    public ResponseEntity<ApiError> handleAccessDenied(Exception e) {
        return buildResponseEntity(new ApiError(HttpStatus.UNAUTHORIZED, "Access denied.",
    ...
    public ResponseEntity<ApiError> handleValidationExceptions(MethodArgumentNotValidException e) {
        ...
        e.getBindingResult().getAllErrors().forEach(error -> stringBuilder.append(String.format("%s : %s ",
                ((FieldError) error).getField(), error.getDefaultMessage())));
        return buildResponseEntity(new ApiError(HttpStatus.BAD_REQUEST, stringBuilder.toString(),
                HttpStatus.BAD_REQUEST.value()));
    }
    ...
    public ResponseEntity<ApiError> handleError(Exception e) {
        LOGGER.error("Exception Caused By: ", e);
        return buildResponseEntity(new ApiError(HttpStatus.INTERNAL_SERVER_ERROR,
                e.getClass().getName() + " " + e.getMessage(), 500));
    }
